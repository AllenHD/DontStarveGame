...
ACTIONS=
{
    ...
    TAKEITEM = Action(),
    MAKEBALLOON = Action(),
    CASTSPELL = Action(0, false, true, 15),
}
...
ACTIONS.CASTSPELL.fn = function(act)
    ...
    if staff and staff.components.spellcaster and staff.components.spellcaster:CanCast(act.doer, act.target) then
        staff.components.spellcaster:CastSpell(act.target)
        return true
    end
