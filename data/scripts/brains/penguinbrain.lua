		    ...
		    lst = FindItems(inst, SEE_DIST/2, function(item) 
                                                    if  item.components.inventoryitem and 
                                                        ...
                                                        (item.HasTag == "penguin_egg" or item.prefab == "bird_egg") then
                                                            return char:GetDistanceSqToInst(item) <= TOOCLOSE*TOOCLOSE*10
                                                        end
                                                    end)
...
local function LayEggAction(inst)
    ...
    elseif not egg then
        if not inst.nesting then  -- egg laying season over
            return
        end
    ...
    local root = PriorityNode(
    {
     ...
     -- DoAction(inst, getactionfn, name, run)
        ...
        WhileNode(function()
                    ...
                    "Migrating ", 
                    DoAction(self.inst, GoHomeAction, "Migrating Action", false )),
